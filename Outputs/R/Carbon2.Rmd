...
Mortality_Sum = Mortality %>% group_by(Disturbancy, Scenario, Run) %>%
      summarise(MortalitySum = sum(Mortality))

InvBessel = (ncol(Mortality_Sum)-1)/ncol(Mortality_Sum) %>% sqrt()

Mortality_Agg = Mortality_Sum %>% group_by(Disturbancy, Scenario) %>%
...
Mortality_AggT = Mortality %>% group_by(Time, Disturbancy, Scenario) %>%
      summarise(MortalityMean = mean(Mortality), MortalitySd = sd(Mortality)*InvBessel)
...
ggplot(Mortality_AggT, aes(x=Time, y=MortalityMean, col=Disturbancy, group=Disturbancy)) +
  ...
  coord_cartesian(ylim=c(-250000000, 1750000000)) +
...
kable(as.data.frame(Mortality_Agg[Mortality_Agg$Disturbancy == "Harvest",]),
      caption = "Biomass harvested depending on the scenario",
      col.names = c("Disturbancy", "Scenario", "Mean biomass harvested", "Standard deviation inter-model"))
...
kable(as.data.frame(Mortality_Agg[Mortality_Agg$Disturbancy == "Insects",]),
      caption = "Insect mortality depending on the scenario",
      col.names = c("Disturbancy", "Scenario", "Mean mortality", "Standard deviation inter-model"))
...
```{r}
kable(as.data.frame(Mortality_Agg[Mortality_Agg$Disturbancy == "Fire accidental",]),
...
```{r FireNat}
ggplot(Mortality_Agg[Mortality_Agg$Disturbancy=="Fire lightning",], aes(x=Scenario, y=MortalityMean, fill=Scenario)) +
geom_bar(stat="identity") +
geom_errorbar(aes(ymin=MortalityMean-MortalitySd, ymax=MortalityMean+MortalitySd)) +
ylab("Biomass burned") +
...
kable(as.data.frame(Mortality_Agg[Mortality_Agg$Disturbancy == "Fire lightning",]),
...
plot(rasTot, legend.only=TRUE, col=pal.2(nbrks),
    legend.width=1, legend.shrink=0.75,
    legend.args=list(text="Proportion of years with fire",
                     side=4, font=2, line=2.5, cex=0.8))
